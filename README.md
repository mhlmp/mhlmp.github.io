...
* **מנוע סריקה מרובה ספקים:** תמיכה מובנית ב-API של מגוון ספקי בינה מלאכותית. המערכת מאפשרת מעבר מהיר בין המנועים במקרה של ניצול מכסות (Rate Limits).
* **חיווי זהות מודל (AI Traceability):** המערכת שומרת ומציגה ויזואלית איזה מנוע AI ביצע את הפיענוח הספציפי על גבי כל כרטיסייה (במקום אייקון ה-🤖 הגנרי). רחרוח (Hover) על הסמל יציג Tooltip עם שם המודל המדויק (לדוגמה `Llama 3` או `Gemini 1.5 Flash`):
  * 🔹 **Google Gemini**
  * ⚡ **Groq**
  * 🌐 **OpenRouter**
* **פענוח אוטומטי של סטטוס הרשת:** ה-AI מונחה לזהות ולהחזיר סטטוס מדויק לכל קישור ולהציג חיווי ויזואלי בהתאם:
  * 🟢 **פעיל (Active):** אתר תקין וזמין.
  * 🟠 **חונה/דומיין (Parked):** דומיין למכירה או דף ללא תוכן ממשי.
  * 🔴 **לא זמין (Dead):** אתר שקרס או קישור שבור.
